...
public class CategoryPanel extends JPanel {
    ...
    public CategoryPanel() {
        ...
        btnAdd.addActionListener(e -> {
            ...
            if(!name.isEmpty()) { Category c = new Category(); c.setName(name); categoryDAO.saveOrUpdate(c); txtName.setText(""); cargar(); }
        });
        btnDelete.addActionListener(e -> {
            int row = table.getSelectedRow();
            if(row != -1) { Category c = new Category(); c.setId((Integer)model.getValueAt(row, 0)); categoryDAO.delete(c); cargar(); }
        });
        cargar();
    }
    private void cargar() {
