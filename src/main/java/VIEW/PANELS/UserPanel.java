...
public class UserPanel extends JPanel {
    ...
    public UserPanel() {
        ...
        gbc.gridx = 1; txtEmail = new JTextField(15); formPanel.add(txtEmail, gbc);

        JPanel btnBox = new JPanel(new FlowLayout(FlowLayout.RIGHT));
        btnAdd = new JButton("Añadir"); btnDelete = new JButton("Eliminar");
        btnBox.add(btnDelete); btnBox.add(btnAdd);
        gbc.gridx = 0; gbc.gridy = 2; gbc.gridwidth = 2; formPanel.add(btnBox, gbc);
        ...
        btnAdd.addActionListener(e -> {
            String name = txtName.getText().trim(); String email = txtEmail.getText().trim();
            if(!name.isEmpty() && !email.isEmpty()) { User u = new User(); u.setName(name); u.setEmail(email); userDAO.saveOrUpdate(u); txtName.setText(""); txtEmail.setText(""); cargar(); }
        });
        btnDelete.addActionListener(e -> {
            int row = table.getSelectedRow();
            if(row != -1) { User u = new User(); u.setId((Integer)model.getValueAt(row, 0)); userDAO.delete(u); cargar(); }
        });
        cargar();
    }
    private void cargar() {
